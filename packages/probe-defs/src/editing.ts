...
export const editingProbes: ProbeDefinition[] = [
  ...
  probe(
    "editing.sl",
    (ctx) => {
      ctx.feed("\x1b[1;1H\x1b[2K") // clear line
      ctx.feed("ABCDEFGH")
      ctx.feed("\x1b[2 @") // SL 2
      const text = ctx.getText()
      // Sequence consumed if no literal " @" leaked into output
  ...
  probe(
    ...
    (ctx) => {
      ...
      ctx.feed("ABCDEFGH")
      ctx.feed("\x1b[2 A") // SR 2
      const text = ctx.getText()
      // Sequence consumed if no literal " A" leaked into output
      const noLeak = !text.includes(" A")
      return {
        pass: noLeak,
        note: noLeak ? "sequence consumed" : "literal leak detected",
      }
