...
Technically, "raw mode" is not a single flag but a collection of termios settings. The `stty raw` command disables input processing (`icanon`), signal generation (`isig`), and output processing (`opost`), among others — but notably it does _not_ disable echo, which is why the classic incantation is `stty raw -echo`. The C library function `cfmakeraw()` (a BSD/glibc extension) sets the full combination, including clearing echo. Applications typically use `tcgetattr()` to save the current settings, call `cfmakeraw()` to switch to raw mode, and call `tcsetattr()` with the saved settings on exit to restore the terminal.
