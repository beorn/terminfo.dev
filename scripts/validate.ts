...
const annotations = loadJson(join(contentDir, "annotations.json")) as Record<
  string,
...
// 7. Terminals with no probe data
{
  ...
  for (const [id, term] of Object.entries(terminals)) {
    ...
    const hasProbe =
      probeBackends.has(id) ||
      backends.some((b) => probeBackends.has(b));
    if (!hasProbe) {
...
// 14. Terminal counts
{
  ...
  for (const [id, term] of Object.entries(terminals)) {
    const backends = term.headlessBackends ?? [];
    const hasProbe =
      probeBackends.has(id) || backends.some((b) => probeBackends.has(b));
...
console.log();
if (errors > 0) {
  console.log(`${RED}${BOLD}${errors} error${errors === 1 ? "" : "s"}${RESET}, ${YELLOW}${warnings} warning${warnings === 1 ? "" : "s"}${RESET}`);
} else if (warnings > 0) {
